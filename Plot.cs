		...
		internal void Plot()
		{
			...
			if (Model.LinFit)
			{
				...
				if (Monotonic(c[1], c[2], c[3]))
				{
					...
					Model.XYprop2 = $"cubic: {c[0]:#0.0000}+{c[1]:#0.0000}*x+{c[2]:#0.0000}*x**2+{c[3]:#0.000000}*x**3 "
								  + $"slopes:  {slope[0]:#0.00000}, {slope[1]:#0.00000}@{inflection:#0.0000}, {slope[2]:#0.00000}";
				}
					...
					if (Monotonic(Ft.Item2, Ft.Item3, Ft.Item4) || true)
					{
						Model.XYprop2 = $"  constrained cubic:  {Ft.Item1:#0.0000} + {Ft.Item2:#0.000000}*x "
								  ...
								  + $";  slopes:  {slope[0]:#0.0000}, {slope[1]:#0.00000}@{inflection:#0.0000}, {slope[2]:#0.0000}";
						model.Series.Add(new FunctionSeries(ccubicfit, xmin, xmax,
										 (xmax - xmin) / 50, "constrained cubic fit"));
					}
					else Model.XYprop2 = "cubic fits failed!";
				}
